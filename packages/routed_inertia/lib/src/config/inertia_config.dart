...
/// Defines history settings applied to Routed Inertia responses.
class InertiaHistoryConfig {
  /// Creates history settings for Routed Inertia responses.
  const InertiaHistoryConfig({required this.encrypt});

  /// Whether history state should be encrypted by default.
  final bool encrypt;
}

/// Defines how Routed resolves Vite client assets.
class InertiaAssetsConfig {
  /// Creates asset settings for Routed Inertia HTML responses.
  ...
  /// The Vite manifest path used for production HTML responses.
  final String? manifestPath;

  /// The manifest entry used when resolving scripts and styles.
  final String? entry;

  /// The public base URL used for resolved asset tags.
  final String baseUrl;

  /// The Vite hot file path used to discover the dev server URL.
  final String? hotFile;

  /// An explicit dev server origin such as `http://127.0.0.1:5173`.
  final String? devServerUrl;

  /// The dev server host used when [devServerUrl] is omitted.
  final String? devServerHost;

  /// The dev server port used when [devServerUrl] is omitted.
  final int? devServerPort;

  /// The dev server scheme used with [devServerHost] and [devServerPort].
  final String devServerScheme;

  /// Resolves the effective Vite dev server URL.
  ///
  /// This uses [devServerUrl] first, then the configured [hotFile], and
  /// finally [devServerHost] with [devServerPort].
...
/// Groups the Routed-specific Inertia configuration loaded from config files.
class InertiaConfig {
  /// Creates a resolved Routed Inertia configuration object.
  InertiaConfig({
  ...
  /// The asset version returned with page responses.
  final String version;

  /// The default template name used for initial HTML responses.
  final String? rootView;

  /// The default history settings applied to responses.
  final InertiaHistoryConfig history;

  /// The SSR settings used to create the default gateway.
  final InertiaSsrSettings ssr;

  /// The asset settings used to resolve scripts, styles, and dev server URLs.
  final InertiaAssetsConfig assets;

  /// An optional callback that overrides [version].
  final String Function()? versionResolver;

  /// An optional SSR gateway override.
  final SsrGateway? ssrGateway;

  /// Resolves the version string sent with Inertia responses.
...
/// Parses and validates Routed Inertia configuration files.
class InertiaConfigSpec extends ConfigSpec<InertiaConfig> {
  /// Creates the Routed Inertia config specification.
